...
public enum ReferenceForExpressionEvaluations {
	...
	PERLINCOLOR {
		/**
		 * Generate Perlin noise based on the given values. Algorithm described
		 * in detail at this site:
		 * http://freespace.virgin.net/hugo.elias/models/m_perlin.htm
		 */
		public RGBColor evaluate(RGBColor left, RGBColor right) {
			double red = ImprovedNoise.noise(left.getRed() + 0.3, right
					.getRed() + 0.3, 0);
			double blue = ImprovedNoise.noise(left.getBlue() + 0.1, right
					.getBlue() + 0.1, 0);
			double green = ImprovedNoise.noise(left.getGreen() - 0.8, right
					.getGreen() - 0.8, 0);
			return new RGBColor(red, green, blue);
		}
	},
	...
	RGB2YCRCR {
		/**
		 * Convert color from RGB to YUV color space. Details and constants
		 * derived from this site: http://www.answers.com/topic/yuv
		 */
		public RGBColor evaluate(RGBColor c, RGBColor unused) {
			double red = c.getRed() * 0.2989 + c.getGreen() * 0.5866
					+ c.getBlue() * 0.1145;
			double green = c.getRed() * -0.1687 + c.getGreen() * -0.3312
					+ c.getBlue() * 0.5;
			double blue = c.getRed() * 0.5000 + c.getGreen() * -0.4183
					+ c.getBlue() * -0.0816;
			return new RGBColor(red, green, blue);
		}
	},
	YCRC2RGBR {
		/**
		 * Convert color from YUV to RGB color space. Details and constants
		 * derived from this site: http://www.answers.com/topic/yuv
		 */
		public RGBColor evaluate(RGBColor c, RGBColor unused) {
			double red = c.getRed() + c.getBlue() * 1.4022;
			double green = c.getRed() + c.getGreen() * -0.3456 + c.getBlue()
					* -0.7145;
